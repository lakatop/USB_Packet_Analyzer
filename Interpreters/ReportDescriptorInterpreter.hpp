...
private:
	std::size_t ParseReportDescriptor(ReportDescTreeStruct* root, const std::size_t parsed);
	void InterpretReportDescriptor(TreeItem* parent, ReportDescTreeStruct* reportDescriptor);
	void InterpretInputOutputFeature(const BYTE tag, const QByteArray& data, TreeItem* child);
